...
describe('computePrediction', () => {
  ...
  it('still computes a nonzero, un-forced mealCOB from logged carbs even when total history is under the 36-point threshold, as long as the recent ~45m window is intact', () => {
    const now = Date.now();
    const result = computePrediction({
      settings: COMPLETE_SETTINGS,
      // Only 1h of history (12 points) — well under MIN_GLUCOSE_POINTS_FOR_COB (36),
      // but cob.js's own bucketing window only needs the last ~45m, which is present.
      glucoseReadings: flatGlucoseHistory(115, 12, now),
      treatments: [{ insulin: null, carbs: 40, createdAt: new Date(now - 20 * 60_000).toISOString() }],
      basalDoses: [],
      now: new Date(now),
    });
    expect(result.status).toBe('ok');
    expect(result.insufficientGlucoseForCOB).toBe(true);
    // The old behavior force-zeroed mealCOB whenever insufficientGlucoseForCOB was
    // true, discarding a perfectly valid cob.js calculation. It must not anymore.
    expect(result.mealCOB).toBeGreaterThan(0);
    expect(result.cobPending).toBe(false);
  });

  it('flags cobPending (rather than looking like confirmed zero carbs) when logged carbs exist but a recent CGM gap prevents absorption confirmation', () => {
    const now = Date.now();
    // 3.5h of otherwise-full history, but nothing in the last 50 minutes —
    // simulates a real BLE/sync gap right when the dashboard is checked.
    const readings = [];
    for (let i = 0; i < 42; i++) {
      const minutesAgo = i * 5;
      if (minutesAgo < 50) continue;
      readings.push(readingAt(115, minutesAgo, now));
    }
    const result = computePrediction({
      settings: COMPLETE_SETTINGS,
    ...
    expect(result.status).toBe('ok');
    expect(result.mealCOB).toBe(0);
    expect(result.cobPending).toBe(true);
  });
  ...
  // determine-basal.js's "CGM data is unchanged, doing nothing" shortcut
  // (see the test above's comment) is common in real use — many CGM
  // sources report a genuine 0 delta between consecutive readings — so
  // predBGs can't just go missing whenever it fires; a flat continuation
  // is oref0's own conclusion here, not a guess.
  it('falls back to a flat predBGs line when determine-basal.js short-circuits on unchanging CGM data', () => {
    const now = Date.now();
    const result = computePrediction({
      settings: COMPLETE_SETTINGS,
      glucoseReadings: flatGlucoseHistory(115, 84, now),
      treatments: [],
      basalDoses: [],
      now: new Date(now),
    });
    expect(result.status).toBe('ok');
    if (result.status === 'ok') {
      expect(result.predBGs).not.toBeNull();
      expect(result.predBGs.every((v) => v === 115)).toBe(true);
    }
  });
});
