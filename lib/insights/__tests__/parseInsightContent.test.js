...
describe('parseInsightContent', () => {
  it('uses an already-structured object directly, tolerating a plain-string pattern', () => {
    ...
    expect(result.structured).toEqual({
      summary: 'Good week overall.',
      patterns: [{ observation: 'Afternoon highs on weekdays', confidence: null }],
      considerations: ['Consider adjusting lunch insulin timing'],
  ...
  it('leaves confidence null when a pattern has no confidence field', () => {
    const result = parseInsightContent({
      summary: 'x',
      patterns: [{ observation: 'Something happened' }],
      considerations: [],
      doctor_discussion_topics: [],
    });
    expect(result.structured.patterns).toEqual([{ observation: 'Something happened', confidence: null }]);
  });
