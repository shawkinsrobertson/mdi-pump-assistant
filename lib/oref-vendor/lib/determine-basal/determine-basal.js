...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    // predictive low glucose suspend mode: BG is / is projected to be < threshold
        ...
        // >>> MDI-FORK (not in upstream oref0): begin >>>>>>>>>>>>>>>>>>>>>>>>>>
        // Predictive low glucose suspend: oref0 would zero a pump's temp
        // basal here. There's no MDI equivalent, so convert the same
        // worstCaseInsulinReq a pump would withhold into a carb suggestion
        // instead. See MODIFICATIONS.md.
        if (profile.mdiMode) {
            var excessInsulin = round(worstCaseInsulinReq, 2);
            var carbsSuggested = round(excessInsulin * profile.carb_ratio, 1);
            rT.reason += ", MDI: fixed basal can't be reduced. Suggesting " + carbsSuggested + "g carbs to cover " + excessInsulin + "U excess basal effect (predictive low). ";
            rT.carbsSuggested = carbsSuggested;
            rT.mdiExcessInsulin = excessInsulin;
            return rT;
        }
        // <<< MDI-FORK: end <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    if (eventualBG < min_bg) { // if eventual BG is below target:
        ...
        if ( minDelta > expectedDelta && minDelta > 0 && !carbsReq ) {
            ...
            if (naive_eventualBG < 40) {
                ...
                // >>> MDI-FORK (not in upstream oref0): begin >>>>>>>>>>>>>>>>>>>>>>>>>>
                // Severe predicted low: oref0 would set a flat 30m zero
                // temp here without computing a magnitude (this case is
                // urgent enough it doesn't bother). For MDI there's still
                // no way to withhold the fixed basal, so derive the same
                // kind of excess-insulin figure used elsewhere in this file
                // (a plain (target - predicted)/sens correction, no extra
                // safety multiplier) and convert it to carbs. See
                // MODIFICATIONS.md.
                if (profile.mdiMode) {
                    var excessInsulin = round((target_bg - naive_eventualBG) / sens, 2);
                    var carbsSuggested = round(excessInsulin * profile.carb_ratio, 1);
                    rT.reason += "MDI: fixed basal can't be reduced. Suggesting " + carbsSuggested + "g carbs to cover " + excessInsulin + "U excess basal effect (severe predicted low). ";
                    rT.carbsSuggested = carbsSuggested;
                    rT.mdiExcessInsulin = excessInsulin;
                    return rT;
                }
        ...
        if (typeof currenttemp.rate !== 'undefined' && (currenttemp.duration > 5 && rate >= currenttemp.rate * 0.8)) {
            ...
            if ( rate <=0 ) {
                ...
                // >>> MDI-FORK (not in upstream oref0): begin >>>>>>>>>>>>>>>>>>>>>>>>>>
                // MDI users have a fixed/steady basal (from long-acting insulin) that
                // can't be temporarily lowered like a pump's temp basal. When the
                // algorithm determines the basal effect needs to be reduced (rate <= 0),
                // there's no way to "un-inject" the long-acting dose already on board.
                // Instead, surface the excess basal effect as a carb suggestion: convert
                // the insulin the fixed basal is now over-delivering (relative to what's
                // needed) into grams of carbs, via the user's own carb ratio, so they can
                // eat to offset it rather than receive a (nonexistent) temp-basal cut.
                // See MODIFICATIONS.md for full rationale.
                if (profile.mdiMode) {
                    var excessInsulin = round(-insulinReq, 2); // insulinReq is <= 0 here; flip sign to a positive "too much basal" amount
                    var carbsSuggested = round(excessInsulin * profile.carb_ratio, 1);
                    rT.reason += ", MDI: fixed basal can't be reduced. Suggesting " + carbsSuggested + "g carbs to cover " + excessInsulin + "U excess basal effect. ";
                    rT.carbsSuggested = carbsSuggested;
                    rT.mdiExcessInsulin = excessInsulin;
                    return rT;
                }
